...
public class KeywordsOptimizeFactory
{
  ...
  private static void initdata() {
    ...
    if ((keyword == null) || 
      ...
      try
      {
        List result = dao.getKeywordsOptimize();
        if (result.size() > 0) {
          keyword = new HashMap();
  ...
  {
    if (instance == null)
      instance = new KeywordsOptimizeFactory();
    return instance;
  }
  ...
  public String getKeopTitle(int keotId, Map param)
  {
    ...
    if ((keyword != null) && 
      ...
      KeywordsOptimize key = 
        (KeywordsOptimize)keyword.get(new Integer(keotId));
