  ...
  extends TagSupport
{
  ...
  public int doEndTag() throws JspException {
    ...
    try {
      out.print(SplitPage(this.current_page, this.count_page, this.url, response));
    }
    catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public String SplitPage(int current_page, int count_page, String url, HttpServletResponse response) {
    ...
    if (current_page > 0) {
      if (current_page != 1)
      {
        pre_page = "[ <a href='" + response.encodeURL(new StringBuilder(String.valueOf(url)).append(current_page - 1).toString()) + "'><span class='H1'>上一页</span></a> ]";
        number_page = pre_page + "&nbsp;&nbsp;" + number_page;
      }
      if (current_page != count_page)
      {
        next_page = "[ <a href='" + response.encodeURL(new StringBuilder(String.valueOf(url)).append(current_page + 1).toString()) + "'><span class='H1'>下一页</span></a> ]";
