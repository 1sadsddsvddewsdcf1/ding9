  ...
  public String SplitPage(int current_page, int count_page, String url)
  {
    ...
    for (int i = 1; i <= 2; i++) {
      if (i < count_page - 1) {
        ...
        if (i == current_page) {
          ...
          number_page_one = number_page_one + "<a href='" + UrlRewriteManager.dynamic2static(new StringBuilder(String.valueOf(url)).append(i).toString()) + "'><span class='H1'>" + i + "</span></a> ";
        }
      }
    }
    for (int i = current_page - 2; i <= current_page + 2; i++) {
      if ((i <= count_page) && 
        ...
        if (i == current_page) {
          ...
          number_page_two = number_page_two + "<a href='" + UrlRewriteManager.dynamic2static(new StringBuilder(String.valueOf(url)).append(i).toString()) + "'><span class='H1'>" + i + "</span></a> ";
        }
      }
    }
    ...
    for (int i = count_page - 1; i <= count_page; i++) {
      if ((i > 0) && (i > last)) {
        count_three++;
        if (i != current_page) {
          number_page_three = number_page_three + "<a href='" + UrlRewriteManager.dynamic2static(new StringBuilder(String.valueOf(url)).append(i).toString()) + "'><span class='H1'>" + i + "</span></a> ";
        } else {
