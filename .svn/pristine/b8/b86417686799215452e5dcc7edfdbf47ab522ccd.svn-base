...
public class KeyUtil
{
  ...
  private static boolean isLowerCaseString(String alpha)
  {
    ...
    for (int j = 0; j < chars.length; j++) {
      if (!java.lang.Character.isLowerCase(chars[j])) { //2014-02-09 if (!Character.isLowerCase(chars[j])) {
        return false;
      }
    }
    return true;
  ...
  public static String toAlphaString(String original)
  {
    ...
    for (int i = 0; i < chinaeseChar.length; i++) {
      ...
      if (java.lang.Character.isWhitespace(chinaeseChar[i])) { //2014-02-09 if (Character.isWhitespace(chinaeseChar[i]))
